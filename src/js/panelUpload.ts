...
function gatherData ( $ ) {
	...
	data.modules.datatables = {
		...
		"builder": ! $.fn.dataTable.ext ?
			false :
			$.fn.dataTable.ext.builder || false
	};
...
function stringify ( $, data ) {
	...
	try {
		...
		alert( "An error occurred, likely due to a circular reference. Please report to support with a link." );
		return;
	}
...
function upload( $, dataStr ) {
	$.ajax( {
		...
		success: function (response) {
			...
			if ( a[0] === 'done' ) {
				$('div.datatables-debug--upload-ready').html( 'Upload complete - <a href="https://debug.datatables.net/'+url+'">'+url+'</a>' );
				$('div.datatables-debug--upload-ready').after( 'Your debug code is shown above. Please include this in any support requests.' );
			}
			else {
