...
export default function(panel, $) {
	...
	panel.html('Running tests...').append(info);
	...
	if (results.length === 0) {
		panel.html(
			tests.length +
				' tests complete. No failures or warnings found!<br><br>If you are having problems with your DataTables, please upload a data profile using the <i>Upload</i> option below, and post a support request in <a href="https://datatables.net/forums">DataTables forums</a>, with a link to a page showing the issue so we can help to debug and investigate the issue.'
		);
	} else {
		panel.html(
			tests.length +
